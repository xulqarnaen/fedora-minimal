...
setup_noctalia_selinux() {
    echo ""
    echo "--- Noctalia Greeter SELinux Setup ---"

    # The policy is only needed when SELinux is enabled. Fedora's default
    # enforcing mode is handled here; disabled systems are left untouched.
    if ! command -v getenforce >/dev/null 2>&1; then
        echo "SELinux tools are not available; skipping Noctalia Greeter SELinux policy."
        return 0
    fi
    ...
    case "$selinux_mode" in
        Disabled)
            echo "SELinux is disabled; skipping Noctalia Greeter SELinux policy."
            return 0
            ;;
        Enforcing|Permissive)
            ;;
        *)
            echo "${RED}ERROR: Unknown SELinux state: $selinux_mode${ALL_OFF}" >&2
            return 1
            ;;
    esac

    local policy_dir="$REPO_DIR/.selinux"
    local policy_file="$policy_dir/noctalia-greeter.te"
    local contexts_file="$policy_dir/noctalia-greeter.fc"
    local module_name="noctalia-greeter"
    local tmp_dir
    local checkpolicy_was_installed=1
    local packages_to_install=()

    if [[ ! -f "$policy_file" || ! -f "$contexts_file" ]]; then
        echo "${RED}ERROR: Noctalia Greeter SELinux policy files are missing from .selinux/.${ALL_OFF}" >&2
        return 1
    fi
    ...
    if ! command -v semodule >/dev/null 2>&1 ||
       ...
       ! command -v restorecon >/dev/null 2>&1; then
        packages_to_install+=(policycoreutils)
    fi

    if ! command -v checkmodule >/dev/null 2>&1; then
        packages_to_install+=(checkpolicy)
        checkpolicy_was_installed=0
    fi

    if ((${#packages_to_install[@]})); then
        echo "Installing required SELinux utilities..."
        if ! dnf --setopt=install_weak_deps=False install -y "${packages_to_install[@]}"; then
            echo "${RED}ERROR: Failed to install SELinux utilities.${ALL_OFF}" >&2
            return 1
        fi
    fi
    ...
    if ! checkmodule -M -m \
        -o "$tmp_dir/$module_name.mod" \
        "$policy_file" ||
       ! semodule_package \
        -o "$tmp_dir/$module_name.pp" \
        -m "$tmp_dir/$module_name.mod" \
        -f "$contexts_file" ||
       ! semodule -i "$tmp_dir/$module_name.pp" ||
       ! restorecon -RF /var/lib/noctalia-greeter; then

        rm -rf "$tmp_dir"

        if ((checkpolicy_was_installed == 0)); then
            dnf remove -y checkpolicy >/dev/null 2>&1 || true
        fi

        echo "${RED}ERROR: Failed to install Noctalia Greeter SELinux policy.${ALL_OFF}" >&2
        return 1
    fi

    rm -rf "$tmp_dir"

    # checkmodule is only needed while building the local module. Avoid
    # leaving a development tool behind when the system did not already have it.
    if ((checkpolicy_was_installed == 0)); then
        if ! dnf remove -y checkpolicy >/dev/null 2>&1; then
            echo "Warning: Could not remove temporary checkpolicy package."
        fi
    fi

    echo "Noctalia Greeter SELinux policy installed."
}

if ! setup_noctalia_selinux; then
    echo "${RED}ERROR: Noctalia Greeter SELinux setup failed.${ALL_OFF}" >&2
    exit 1
fi
